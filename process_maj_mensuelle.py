...
CATEGORY_MODEL_NAME = "Random Forest Bottom-Up"
...
# CALENDRIER FUTUR POUR HORIZONS
...
# CATEGORISATION POUR LIGHTGBM

def align_lgb_categories(X_train: pd.DataFrame,
                         X_predict: pd.DataFrame,
                         categorical_cols):
    """Aligne strictement les catégories pandas entre train et predict."""
    ...
    for col in categorical_cols:
        ...
        if isinstance(X_train[col].dtype, pd.CategoricalDtype):
            ...
            train_s = X_train[col].astype("string")
            categories = pd.Index(train_s.dropna().unique())
...
# 6. ENTRAINEMENT ET FORECAST — SITE x SUPPLIER x CATEGORY LIGHTGBM RESIDUAL
...
def train_and_forecast_site(panel_lgb, forecast_origin_df, forecast_origin, horizons):
    ...
    for h in horizons:
        ...
        pred_change = model.predict(X_forecast_lgb)
...
# RANDOM FOREST BOTTOM-UP — SUPPLIER x CATEGORY
...
def make_rf_pipeline(features):
    ...
    preprocessing = ColumnTransformer([
        ("numeric", numeric_pipeline, numeric_now),
        ("categorical", categorical_pipeline, categorical_now),
    ])
...
def train_and_forecast_category_bottom_up(panel: pd.DataFrame,
                                          forecast_origin_df: pd.DataFrame,
    ...
    panel_rf = panel.copy()
    forecast_rf = forecast_origin_df.copy()

    for col in CATEGORICAL_FEATURES:
        if col in panel_rf.columns:
            panel_rf[col] = panel_rf[col].astype("string")
        if col in forecast_rf.columns:
            forecast_rf[col] = forecast_rf[col].astype("string")

    for h in horizons:
        log("\n" + "=" * 80)
        log(f"TRAINING CATEGORY BOTTOM-UP RF — H{h}")
        log("=" * 80)

        target = f"target_h{h}"
        features = get_features(panel_rf, h)
        ...
        model = make_rf_pipeline(features)
        ...
        site_prediction = model.predict(
            forecast_rf[features]
        )
        site_prediction = np.maximum(
            site_prediction,
            0,
        )

        internal_site = forecast_rf[
            SERIES_COLS + ["quantity_current"]
        ].copy()
        internal_site["forecast_quantity"] = site_prediction
        ...
        aggregate = (
            ...
            .agg(
                quantity_current=("quantity_current", "sum"),
                forecast_quantity=("forecast_quantity", "sum"),
            )
        )

        aggregate["forecast_origin"] = forecast_origin
        aggregate["horizon"] = h
        aggregate["target_month"] = (
            forecast_origin + pd.DateOffset(months=h)
        )
        aggregate["model"] = CATEGORY_MODEL_NAME

        aggregate_blocks.append(aggregate)

    aggregate_forecast = pd.concat(
        aggregate_blocks,
        ignore_index=True,
    )
    ...
    log(
        "supplier_category bottom-up forecast shape:",
        aggregate_forecast.shape,
    )

    return aggregate_forecast, training_rows_by_horizon


# STANDARDIZER L'OUTPUT
...
# UPLOAD ml_demand_forecast_base.csv ----
    df = load_data(INPUT_FILE)
 
# DETECTER forecast_origin
  
    forecast_origin = df["month_date"].max()
    log("\nDetected forecast_origin:", forecast_origin.date())
 
 #UPLOAD HISTORIQUE EXISTANT DE NOS TABLES OUTPUT PRECEDENT
...
# MISE A JOUR FORECAST ET COMPARAISON
...
#RECALCULER LES METRIQUES DE SUIVI
...
#RECONSTRUCTION DU PANEL ET DES FEATURES
...
#ENTRAINEMENT LIGHTGBM RESIDUAL NIVEAU SITE
...
#FORECAST BOTTOM-UP RANDOM FOREST
  
    log("\nTraining Random Forest H1-H6 & building SUPPLIER_CATEGORY bottom-up forecast...")
    aggregate_forecast, rf_training_rows_by_horizon = train_and_forecast_category_bottom_up(
        panel,
        forecast_origin_df,
        forecast_origin,
        FORECAST_HORIZONS,
...
#ENREGISTRER LE FORECAST EN HISTORIQUE
    ...
    all_training_rows = {
        f"LGBM_H{h}": n
        for h, n in training_rows_by_horizon.items()
    }
    all_training_rows.update({
        f"RF_H{h}": n
        for h, n in rf_training_rows_by_horizon.items()
    })
...
#CHECKS
    ...
    site_current = new_forecast_rows[
        new_forecast_rows["forecast_level"] == FORECAST_LEVEL_SITE
    ]
    category_current = new_forecast_rows[
        new_forecast_rows["forecast_level"] == FORECAST_LEVEL_SUPPLIER_CATEGORY
    ]

    expected_h = {f"H{h}" for h in FORECAST_HORIZONS}

    assert set(site_current["horizon"].unique()) == expected_h
    assert set(category_current["horizon"].unique()) == expected_h
    assert (site_current["forecast_quantity"] >= 0).all()
    assert (category_current["forecast_quantity"] >= 0).all()
    assert (site_current["model"] == SITE_MODEL_NAME).all()
    assert (category_current["model"] == CATEGORY_MODEL_NAME).all()

    log("Sanity checks: OK")
    log("SITE model:", SITE_MODEL_NAME)
    log("SUPPLIER_CATEGORY model:", CATEGORY_MODEL_NAME)
